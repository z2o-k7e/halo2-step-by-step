...
impl <F: Field> SimpleChip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> SimpleConfig {
        ...
        meta.create_gate("complex_gate", |meta| {
            ...
            Constraints::with_selector(s_cpx, 
                vec![e_cub - out])
        });
    ...
    pub fn assign( 
        ...
        c: F
            ...
            |mut region| {
                ...
                let b_cell = region.assign_advice(|| "private input b",  self.config.advice[1], 0, || b).map(Number)?;
                let c_cell = region.assign_advice_from_constant(|| "private input c",  self.config.advice[2], 0, c).map(Number)?;
            ...
            layouter.assign_region(|| "load witness", move |mut region|{
                ...
                // load a, b
                let (a, b, c) = &cells;
                // we need these cause the s_cpx selector acts on this line. shape of regions must match the custom_gate.
                ...
                let c = c.0.copy_advice(|| "rhs", &mut region, self.config.advice[2], offset).map(Number)?;

                config.s_cpx.enable(&mut region, offset)?;  // Attention the positon of s_cpx to offset.
                offset += 1;

                let e: Value<F> = 
                    (a.0.value().copied() * b.0.value().copied())   // ab
                    * (a.0.value().copied() * b.0.value().copied()) // ab * ab => absq
                    * c.0.value().copied()                          // absq * c => d
                    + c.0.value().copied();                          // d + c => e
                let e_cub = e * e * e;
